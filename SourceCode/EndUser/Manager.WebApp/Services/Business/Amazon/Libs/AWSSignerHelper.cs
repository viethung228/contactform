    ...
    public class AWSSignerHelper
    {
        ...
        public virtual string ExtractCanonicalURIParameters(string resource)
        {
            ...
            else
            {
                ...
                // Encode twice
                encodedSegments = encodedSegments.Select(segment => AWSUtils.UrlEncode(segment));
                encodedSegments = encodedSegments.Select(segment => AWSUtils.UrlEncode(segment));
        ...
        public virtual string ExtractCanonicalQueryString(IRestRequest request)
        {
            ...
            foreach (var key in sortedqueryParameters.Keys)
            {
                ...
                canonicalQueryString.AppendFormat("{0}={1}",
                    AWSUtils.UrlEncode(key),
                    AWSUtils.UrlEncode(sortedqueryParameters[key]));
            }
        ...
        public virtual string HashRequestBody(IRestRequest request)
        {
            ...
            return AWSUtils.ToHex(AWSUtils.Hash(value));
        }
        ...
        public virtual string BuildStringToSign(DateTime signingDate, string hashedCanonicalRequest, string region, string serviceName)
        {
            string scope = BuildScope(signingDate, region, serviceName);
            ...
            )
        {
            ...
            byte[] kDate = AWSUtils.GetKeyedHash(kSecret, date);
            byte[] kRegion = AWSUtils.GetKeyedHash(kDate, region);
            byte[] kService = AWSUtils.GetKeyedHash(kRegion, serviceName);
            byte[] kSigning = AWSUtils.GetKeyedHash(kService, TerminationString);

            // Calculate the signature
            return AWSUtils.ToHex(AWSUtils.GetKeyedHash(kSigning, stringToSign));
        }
        ...
        private static string BuildScope(DateTime signingDate, string region, string serviceName)
        {
