    ...
    $(".item").each(function () {
        if ($(this).attr("id") === control) {
            ...
            if ($(this).hasClass("date")) {
                $(this).find("input").mask("99/99/9999", { placeholder: "yyyy/mm/dd" });               
            }
...
$('#frmChangeAvatar').submit(function () {
    var change = $("#ChangingDetected").val();
    if (change === "y") {
        var params = $.extend({}, doAjax_params_default);
        params['url'] = "/MyAccount/UploadAvatar";
        params['requestType'] = "POST";
        params['data'] = $('#frmChangeAvatar').serialize();
        params['dataType'] = "json";
        params['showloading'] = false;

        params['successCallbackFunction'] = function (data) {
            if (data.success) {
                $.showSuccessMessage(LanguageDic["LB_NOTIFICATION"], data.message, function () { location.reload(); });
                $("#ChangingDetected").val("");
            }
            else {
                $.showErrorMessage(LanguageDic["LB_NOTIFICATION"], data.message, null);
            }
        };
...
$("body").on("click", ".slim-btn-confirm", function () {
    $("#ChangingDetected").val("y");
