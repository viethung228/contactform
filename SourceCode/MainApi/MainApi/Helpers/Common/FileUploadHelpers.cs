...
namespace MainApi.Helpers
{
    public class FileUploadHelper
    {
        ...
        public static async Task<ApiResponseUploadFileModel> UploadImageAsync(IFormFile file, string folderName, bool includeDatePath = false)
        {
            var model = new ApiResponseUploadFileModel();
            ...
            var generatedFileName = Utility.Md5HashingData(System.Guid.NewGuid().ToString());
            ...
            try
            {
                using (var stream = new MemoryStream())
                {
                    ...
                    var myImg = Image.FromStream(stream);
                    ...
                    model.Id = generatedFileName;
                    model.Name = newFileName;
                    model.Path = absolutePath;
                    model.Url = CdnHelper.GetFullFilePath(model.Path);
        ...
        public static async Task<ApiResponseUploadFileModel> UploadFileAsync(IFormFile file, string folderName, bool includeDatePath = false)
        {
            var model = new ApiResponseUploadFileModel();
            ...
            var fileExt = Path.GetExtension(file.FileName);
            //var fileName = TextHelpers.ConvertToUrlFriendly(Path.GetFileNameWithoutExtension(file.FileName), generatedFileName);
            ...
            try
            {
                using (Stream fileStream = new FileStream(Path.GetFullPath(path), FileMode.Create, FileAccess.Write))
                {
        ...
        public static async Task<ApiResponseUploadFileModel> UploadFileCustomAsync(IFormFile file, string fileName, string folderName, bool includeDatePath = false)
        {
            var model = new ApiResponseUploadFileModel();
            ...
            //var generatedFileName = Utility.Md5HashingData(System.Guid.NewGuid().ToString());
            var generatedFileName = System.IO.Path.GetFileNameWithoutExtension(fileName);
        ...
        public static void DeleteMediaFile(string url)
        {            
            try
            {
                var filePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot" + DirSeparator + url);
            ...
            {
                var strError = string.Format("Failed for DeleteMediaFile: {0}", ex.ToString());
                _logger.Error(strError);
            }
        }
