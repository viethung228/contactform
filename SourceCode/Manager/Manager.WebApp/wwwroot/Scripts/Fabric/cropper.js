...
(function (y) {
    y.ImageCropper = function (z, y, E) {
        ...
        //render cropping control image on image editor canvas
        $("#image-editor-container").append(u);
...
(function () {
    ...
    //start crop the active canvas object
    $d.on("click", "#cropping", function () {
        console.log("start cropping")

        var activeObj = imgEditor.canvas.getActiveObject();

        if (activeObj && activeObj.type === "image") {
            f.empty();
            //var c = Math.min(600, $("#ic-main").width());
            //var c = Math.min(600, $("#image-editor-container").width());
            var c = Math.min(600, $(".canvas-container").width());
            ...
            var base64 = activeObj.toDataURL();
            b.setSrc(base64);

            $(".ic-hidden.ic-crop-btn-wrap").removeClass("ic-hidden");

            $("#toolbar .main-buttons").addClass("hidden");

            $("#select-panel").addClass("hidden");
    ...
    //when click done cropping button
    $d.on("click", "#ic-crop-btn", function () {
        ...
        $(".icrop-wrap").addClass("hidden");

        //remove original image from main canvas
        var originalImg = imgEditor.canvas.getActiveObject();
        imgEditor.canvas.remove(originalImg);

        //add cropped image to main canvas
        fabric.Image.fromURL(
            c, function (myImg) {
                ...
                imgEditor.canvas.centerObject(img);
        ...
        $("#toolbar .main-buttons").removeClass("hidden");

        $("#select-panel").removeClass("hidden");
    });

    $d.on("click", "#ic-crop-btn--cancel", function () {
