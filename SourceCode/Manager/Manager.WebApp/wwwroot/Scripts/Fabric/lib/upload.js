...
(function () {
  var upload = function (canvas) {
    ...
    this.openDragDropPanel = function () {
      ...
      //--trigger upload window immediatly when upload image button on toolbar is clicked
    ...
    const processFiles = (files) => {
      ...
      for (let file of files) {
        ...
        // handle image, read file, add to canvas
          reader.onload = (f) => {
              var mainWidth = $("#image-editor-container").width();
              var mainHeight = $("#image-editor-container").height();
          fabric.Image.fromURL(f.target.result, (img) => {
            img.set({
              left: 0,
              top: 0
            })
              img.scaleToHeight(Math.min(300, mainHeight/2))
              img.scaleToWidth(Math.min(300, mainWidth / 2))
